...
impl Drop for INodeImpl {
    ...
    fn drop(&mut self) {
        if self.disk_inode.read().nlinks <= 0 {
            ...
            seg.seg_imap.write().insert(self.id, 0); // indicate this inode is being dumped TODO: fix using INVALID_INO
            drop(seg);
            drop(segments);
            drop(disk_inode);
...
impl LogFileSystem {
    ...
    fn alloc_segment(&self) {
        ...
        if new_seg_id == 0 {
            // println!("enter garbage collecting...");
    ...
    fn alloc_block(&self) -> Option<usize> {
        ...
        if current_seg_size > SEGMENT_SIZE - BLKSIZE {
            ...
            if current_seg_size == SEGMENT_SIZE {
                ...
                // // println!("allocate blkid: {} at segment: {} size: {}", new_blk_id, cur_seg_id, self.segments.read().get(&cur_seg_id).unwrap().meta.size);
            }
        }
        Some(new_blk_id)
    }
    ...
    fn free_block(&self, block_id: usize) {
        ...
        seg.summary_map.write().insert(block_id, SummaryEntry {
            ...
            inode_id: INVALID_INO as i32, // indicate an invalid (unused) block
        });
    ...
    fn _record_block_summary(&self, ino_id: INodeId, blk_id: BlockId, entry_id: isize) {
        ...
        if entry_id == ENTRY_SPECIALBLOCK {
            // // println!("record seg {} ino {} blk {} entry {}", seg_id, ino_id, blk_id, entry_id);
            assert!(seg.summary_map.read().contains_key(&blk_id));
        }
    }
        ...
        for seg_i in 1..self.super_block.read().n_segment as usize {
            ...
            if seg.meta.size == SEGMENT_SIZE as u32 {
                ...
                for (blkid, entry_i) in seg.summary_map.write().iter() {
                    ...
                    // // println!("seg {} blk {} alive {} cleanable {}", seg_i, blkid, alive, cleanable);
                    if alive {
